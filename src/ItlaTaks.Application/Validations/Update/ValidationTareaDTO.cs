...
{
    public class ValidationTareaDTO : AbstractValidator<TareaDTO>
    {
        private readonly TaskContext _context;
        public ValidationTareaDTO(TaskContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.Titulo)
                ...
                .Must((dto, nombre)=>verificateTitle(nombre, dto.Id))
                    .WithMessage("Ya existe una Tarea con ese {PropertyName}");
        ...
        private bool verificateTitle(string title, int id)
        {
            if (_context.Tareas.Any(x => x.Titulo.ToLower().Trim() == title.ToLower().Trim() && x.Id == id))
                return false;
